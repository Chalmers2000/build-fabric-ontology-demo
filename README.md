...
* Fast Path - just grab the notebooks and install a demo in your lakehouse.
...
* For the table creation, your .RDF file already contains table names, columns, and data types, so your request is straightforward. Remember to update references to use your new ontology.rdf file.
* I focused a lot of attention on asking for specific demo data: With a small data set like this, it can be challenging to create interesting multi-hop queries, this data set should respond well.
...
### A PLEASANT SURPRISE
* GitHub Copilot proactively asked me if I wanted an 'orchestration notebook' to create the table-building notebooks. Of course I said yes! That really shows the power of using a good model for this, instead of a default like Haiku 4.6.
...
Moving onto Step 2 and Step 2.5 in the GOALS.MD file, now that I had Lakehouse tables built and populated, we can create a Fabric Ontology.
...
* But teaching Copilot how to author a sempy notebook took a little work. Skills for Fabric does have some authoring skills, but I took it further by asking another LLM to create the sempy-notebook SKILL.md file. Microsoft has done a great job documenting the semantic link library and providing sample notebooks, including the [Microsoft Fabric Community Notebook Gallery](https://community.fabric.microsoft.com/t5/Notebook-Gallery/bd-p/pbi_notebookgallery), so I provided the LLM with both documentation and working samples to help it write a good SKILL.md file. 
* After that, GitHub Copilot easily created the Create_Ontology notebook, I uploaded it, and it built the ontology successfully on the first try.


![Fabric Ontology](images/Fabric_Ontology.png)


* Note that Graph DB takes awhile to completely build: I resized my F64 up to an F128 for this step, it still seems to take 10-15 minutes before it's ready.


## CREATE SOME FUN GRAPH DB QUERIES
The real power of Ontology and graph databases come from making these friend-of-a-friend-of-a-friend-of-a-friend connections. When you get to 4 or 5 hops, these queries become near impossible to run in the relational model, yet take seconds (if that) using Graph.
...
So the challenge was on: Can we create fun Graph queries that show off the unique capabilities of Ontology? 
* Step 3 of the GOALS.MD file asked for those queries.
* I was surprised to see Copilot struggle to write good Graph queries, but looking at the error messages, I realized there's more than one Graph query language on the internet. Microsoft documentation to the rescue! Once I provided the documentation for Fabric's graph language and working examples, I got what I wanted - successful multi-hop graph queries.
* I turned those learnings into the fabric-graphdb-query SKILL.md file so hopefully the next person who has to do this can just reference the skill file and the Copilot will write them out.


## BUILD YOUR DATA AGENT
Building the Data Agent itself was incredibly easy
* One step - add the Ontology as a data source. That's it, you're ready to start prompting the data agent right away. 
